...
class Order extends Component
{
    ...
    public function paymentOrder($paymentToken)
    {
        ...
        if ($this->items->pluck('codigo_id')->first()) {
            $cupon = $this->items->sum('total') * $this->items->pluck('code')->first()->descuento / 100;
            $subtotal = $this->items->sum('total') - $cupon;
        }
        ...
        try {
            ...
            Log::debug("el proceso de pago no fue payment o auth este es el objeto: $aprov");
            $this->dispatch('notify', msj: 'Se produjo un error al realizar su pago', method: 'errorProcessPayment', type: 'error');
        } catch (\Throwable $th) {
    ...
    public function saveReeservationOnly()
    {
        ...
        try {
            ...
            $order = $this->saveReservation($cli, $date, $hour);
            ...
            $this->dispatch('notify', msj: 'Se genero correctamente la reservacion', type: 'success', method: 'registerOrder');
        } catch (\Throwable $th) {
            // Puedes retornar el error como array para mostrarlo en la vista
            $this->dispatch('notify', msj: $th->getMessage(), type: 'error', method: 'registerOrder');
        }
    }

    #[On('store')]
    public function saveReservationPayment()
    {
        $cli = session('cli');
        $date = $this->items->pluck('date')->first();
        $hour = $this->items->pluck('hour')->first();
    ...
    protected function saveReservation($cli, $date, $hour)
    {
